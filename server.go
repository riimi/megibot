...
func (app *KitchenSink) Callback(c echo.Context) error {
	r := c.Request()
	events, _ := app.bot.ParseRequest(r)
	for _, event := range events {
		...
		case linebot.EventTypePostback:
			...
			done := cmd.Manager.SendCmd(cmd.Cmd{
...
func (app *KitchenSink) handleImage(message *linebot.ImageMessage, replyToken string) error {
	return app.handleHeavyContent(message.ID, func(originalContent *os.File) error {
		...
		*/
		...
		file, err := os.Open(originalContent.Name())
		if err != nil {
		...
		var label string
		var i int
		for _, ent := range web.WebEntities {
			if i += 1; i > 3 {
				break
			}
			l, _ := json.Marshal(ent)
			label += string(l) + "\n"
		}

		match, _ := json.Marshal(web.FullMatchingImages)
		if _, err := app.bot.ReplyMessage(
			replyToken,
			linebot.NewTextMessage(fmt.Sprintf("[label]\n%s[match]\n%s", string(label), string(match))),
		).Do(); err != nil {
			return err
		}
		return nil
